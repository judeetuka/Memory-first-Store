...
use mfs_core::s3fifo::{S3FifoCache as MfsS3FifoCache, S3FifoConfig as MfsS3FifoConfig};
...
fn mfs_s3fifo_cache_hit(cache: &MfsS3FifoCache<CacheKey, CacheValue>, keys: &[String]) -> f64 {
    let mut hit = 0;
    for key in keys.iter() {
...
fn new_mfs_s3fifo_hot95(capacity: usize) -> MfsS3FifoCache<CacheKey, CacheValue> {
    MfsS3FifoCache::with_config(MfsS3FifoConfig::new(capacity).with_hot_ratio_percent(95))
}

fn new_mfs_s3fifo_ghost25(capacity: usize) -> MfsS3FifoCache<CacheKey, CacheValue> {
    MfsS3FifoCache::with_config(MfsS3FifoConfig::new(capacity).with_ghost_ratio_percent(25))
}

fn new_mfs_s3fifo_admission(capacity: usize) -> MfsS3FifoCache<CacheKey, CacheValue> {
    MfsS3FifoCache::with_config(
        MfsS3FifoConfig::new(capacity)
            .with_shards(1)
...
fn bench_workload(keys: Vec<String>, cache_size: usize) {
    ...
    let mfs_s3fifo_default_cache = MfsS3FifoCache::with_capacity(cache_size);
    let mfs_s3fifo_hot95_cache = new_mfs_s3fifo_hot95(cache_size);
    let mfs_s3fifo_ghost25_cache = new_mfs_s3fifo_ghost25(cache_size);
    let mfs_s3fifo_admission_cache = new_mfs_s3fifo_admission(cache_size);
    ...
    let mfs_s3fifo_default_cache_hit_handle = std::thread::spawn({
        let cache = mfs_s3fifo_default_cache;
        let keys = keys.clone();
        move || mfs_s3fifo_cache_hit(&cache, &keys)
    });

    let mfs_s3fifo_hot95_cache_hit_handle = std::thread::spawn({
        let cache = mfs_s3fifo_hot95_cache;
        let keys = keys.clone();
        move || mfs_s3fifo_cache_hit(&cache, &keys)
    });

    let mfs_s3fifo_ghost25_cache_hit_handle = std::thread::spawn({
        let cache = mfs_s3fifo_ghost25_cache;
        let keys = keys.clone();
        move || mfs_s3fifo_cache_hit(&cache, &keys)
    });

    let mfs_s3fifo_admission_cache_hit_handle = std::thread::spawn({
        let cache = mfs_s3fifo_admission_cache;
        let keys = keys.clone();
        move || mfs_s3fifo_cache_hit(&cache, &keys)
    });
    ...
    let mfs_s3fifo_default_hit_ratio = mfs_s3fifo_default_cache_hit_handle.join().unwrap();
    let mfs_s3fifo_hot95_hit_ratio = mfs_s3fifo_hot95_cache_hit_handle.join().unwrap();
    let mfs_s3fifo_ghost25_hit_ratio = mfs_s3fifo_ghost25_cache_hit_handle.join().unwrap();
    let mfs_s3fifo_admission_hit_ratio = mfs_s3fifo_admission_cache_hit_handle.join().unwrap();
    ...
    print!("{:21.2}%", mfs_s3fifo_default_hit_ratio * 100.0);
    print!("{:21.2}%", mfs_s3fifo_hot95_hit_ratio * 100.0);
    print!("{:21.2}%", mfs_s3fifo_ghost25_hit_ratio * 100.0);
    print!("{:24.2}%", mfs_s3fifo_admission_hit_ratio * 100.0);
    println!();
}
...
fn bench_zipf_hit() {
    println!(
        "{:30}{:16}{:16}{:16}{:16}{:16}{:16}{:22}{:22}{:22}{:25}",
        "zif_exp, cache_size",
        "fifo",
        "lru",
        "lfu",
