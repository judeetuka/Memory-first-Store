...
fn mfs_s3fifo_get_or_insert(cache: &S3FifoCache<u64, ()>, key: u64) -> bool {
...
fn bench_one(zip_exp: f64, cache_size_percent: f32) {
    ...
    let mfs_s3fifo_default = S3FifoCache::with_capacity(cache_size);
    let mfs_s3fifo_hot95 =
        S3FifoCache::with_config(S3FifoConfig::new(cache_size).with_hot_ratio_percent(95));
    let mfs_s3fifo_ghost25 =
        S3FifoCache::with_config(S3FifoConfig::new(cache_size).with_ghost_ratio_percent(25));
    let mfs_s3fifo_admission = S3FifoCache::with_config(
        S3FifoConfig::new(cache_size)
            .with_shards(1)
    ...
    let mut mfs_s3fifo_default_hit = 0;
    let mut mfs_s3fifo_hot95_hit = 0;
    let mut mfs_s3fifo_ghost25_hit = 0;
    let mut mfs_s3fifo_admission_hit = 0;
    ...
    for _ in 0..ITERATIONS {
        ...
        if mfs_s3fifo_get_or_insert(&mfs_s3fifo_default, key) {
            mfs_s3fifo_default_hit += 1;
        }

        if mfs_s3fifo_get_or_insert(&mfs_s3fifo_hot95, key) {
            mfs_s3fifo_hot95_hit += 1;
        }

        if mfs_s3fifo_get_or_insert(&mfs_s3fifo_ghost25, key) {
            mfs_s3fifo_ghost25_hit += 1;
        }

        if mfs_s3fifo_get_or_insert(&mfs_s3fifo_admission, key) {
            mfs_s3fifo_admission_hit += 1;
        }
    }
    ...
    print!(
        "{:.2}%\t\t",
        mfs_s3fifo_default_hit as f32 / ITERATIONS as f32 * 100.0
    );
    print!(
        "{:.2}%\t\t",
        mfs_s3fifo_hot95_hit as f32 / ITERATIONS as f32 * 100.0
    );
    print!(
        "{:.2}%",
        mfs_s3fifo_ghost25_hit as f32 / ITERATIONS as f32 * 100.0
    );
    println!(
        "\t\t{:.2}%",
        mfs_s3fifo_admission_hit as f32 / ITERATIONS as f32 * 100.0
    );
}
...
fn main() {
    println!(
        "zipf & cache size\t\tlru\t\tmoka\t\tQuickC\t\tTinyUFO\t\tmfs_s3fifo_default\tmfs_s3fifo_hot95\tmfs_s3fifo_ghost25\tmfs_s3fifo_admission",
    );
