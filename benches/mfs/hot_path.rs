...
use mfs_neural::bucketed_index::BucketedIndex;
use mfs_neural::dense_kv::DenseKvMap;
use mfs_neural::dense_writeback::DenseWriteBehindU64;
...
fn populate_dense_writeback(count: u64) -> DenseWriteBehindU64 {
    let cache = DenseWriteBehindU64::with_config(WriteBehindConfig {
    ...
    });
    {
        let p = cache.pin();
        for i in 0..count {
            p.load_clean(i, i.wrapping_mul(2));
        }
    }
    cache
}

fn populate_dense_kv(count: u64) -> DenseKvMap<u64, [u8; 8]> {
    let cache = DenseKvMap::<u64, [u8; 8]>::with_capacity(count as u32);
    {
        let p = cache.pin();
        for i in 0..count {
            p.put(i, i.to_le_bytes()).expect("populate DenseKvMap");
        }
    }
    cache
}

fn populate_dense_writeback_map(count: u64) -> DenseWriteBehindMap<u64, [u8; 8]> {
    let cache = DenseWriteBehindMap::<u64, [u8; 8]>::with_config(WriteBehindConfig {
        initial_capacity: count as usize,
        dirty_queue_capacity: count as usize,
        ..WriteBehindConfig::default()
    });
...
fn main() {
    ...
    println!("=== single-threaded hot paths (count={count}, trials={TRIALS}) ===");
    ...
    // Single-thread write microbench: insert `count` distinct keys into a
    // fresh map. Measures the steady-state insert cost dominated by
    // `Box::new(Entry { ... })` + CAS into the entries slot + meta byte
    // store.
    ...
    // Single-thread replace microbench: pre-populated map, replace each
    // existing key. This is the more realistic write path (most cache
    // writes update existing keys) and exercises the reclamation
    // pathway (the old `Box<Entry>` gets retired on every iteration).
    ...
    measure("bucketed_index_insert", count, || {
        let index = BucketedIndex::<u64>::with_capacity(count as usize);
        for i in 0..count {
            let _ = index.insert_returning_old(black_box(i), black_box(i.wrapping_mul(3)));
        }
        index.len() as u64
    })
    .print();
    let bucketed = BucketedIndex::<u64>::with_capacity(count as usize);
    for i in 0..count {
        let _ = bucketed.insert_returning_old(i, i.wrapping_mul(2));
    }
    measure("bucketed_index_replace", count, || {
        for i in 0..count {
            let _ = bucketed.insert_returning_old(black_box(i), black_box(i.wrapping_mul(11)));
        }
        bucketed.len() as u64
    })
    .print();
    ...
    let dense_writeback = populate_dense_writeback(count);
    measure("dense_writeback_get", count, || {
        let mut checksum = 0u64;
        let p = dense_writeback.pin();
        for i in 0..count {
            checksum ^= p.get(black_box(&i)).expect("loaded key");
        }
        checksum
    ...
    .print();
    measure("dense_writeback_put", count, || {
        let p = dense_writeback.pin();
        for i in 0..count {
            p.put(black_box(i), black_box(i.wrapping_mul(13)));
        }
        dense_writeback.len() as u64
    })
    .print();
    measure("dense_writeback_put_dirty", count, || {
        let p = dense_writeback.pin();
        for i in 0..count {
            p.put(black_box(i), black_box(i.wrapping_mul(17)));
        }
        dense_writeback.len() as u64
    })
    .print();

    let dense_kv = populate_dense_kv(count);
    measure("dense_kv_get", count, || {
        let p = dense_kv.pin();
        let mut checksum = 0u64;
        for i in 0..count {
            checksum ^= u64::from_le_bytes(p.get(black_box(&i)).expect("loaded key"));
        }
        checksum
    })
    .print();
    measure("dense_kv_read_with", count, || {
        let p = dense_kv.pin();
        let mut checksum = 0u64;
        for i in 0..count {
            checksum ^= p
                .read_with(black_box(&i), |value| u64::from_le_bytes(*value))
                .expect("loaded key");
        }
        checksum
    })
    .print();
    measure("dense_kv_put", count, || {
        let p = dense_kv.pin();
        for i in 0..count {
            p.put(black_box(i), black_box(i.wrapping_mul(19).to_le_bytes()))
                .expect("DenseKvMap existing key");
        }
        dense_kv.len() as u64
    })
    .print();

    let dense_writeback_map = populate_dense_writeback_map(count);
    measure("dense_writeback_map_get", count, || {
        let mut checksum = 0u64;
        let p = dense_writeback_map.pin();
        for i in 0..count {
            checksum ^= u64::from_le_bytes(p.get(black_box(&i)).expect("loaded key"));
        }
        checksum
    ...
    .print();
    measure("dense_writeback_map_put", count, || {
        let p = dense_writeback_map.pin();
        for i in 0..count {
            p.put(black_box(i), black_box(i.wrapping_mul(19).to_le_bytes()));
        }
        dense_writeback_map.len() as u64
    })
    .print();
    measure("dense_writeback_map_put_dirty", count, || {
        let p = dense_writeback_map.pin();
        for i in 0..count {
            p.put(black_box(i), black_box(i.wrapping_mul(23).to_le_bytes()));
        }
        dense_writeback_map.len() as u64
    })
    .print();
...
fn multithread_bench(count: u64) {
    ...
    println!("=== multi-threaded read scaling ===");
