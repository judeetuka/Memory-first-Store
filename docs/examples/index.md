...
|---|---|---|---|
| `read_through_cache` | `mfs-core` | Read-through cache pattern: cache-first reads with write-behind flush to a DB backend | `cargo run -p mfs-core --release --example read_through_cache` |
...
| `nosql_raw_kv` | `mfs-store` | Raw bytes, versions, conflicts, and delete against the hot store | `cargo run -p mfs-store --release --example nosql_raw_kv` |
| `nosql_schema_mode` | `mfs-store` | Schema validation plus put/get for typed documents | `cargo run -p mfs-store --release --example nosql_schema_mode` |
| `nosql_wal_recovery` | `mfs-store` | Raw WAL sync and replay for the hot store | `cargo run -p mfs-store --release --example nosql_wal_recovery` |
| `nosql_checkpoint_recovery` | `mfs-store` | Checkpoint plus WAL suffix replay for fast recovery | `cargo run -p mfs-store --release --example nosql_checkpoint_recovery` |
...
- **New to MfS?** Start with `read_through_cache` (core) or `nosql_raw_kv` (store).
- **Need durability?** Run `wal_recovery` (core) or `nosql_wal_recovery` (store).
