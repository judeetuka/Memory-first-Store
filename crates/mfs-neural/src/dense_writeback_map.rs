//! Generic dense write-behind map for byte-stable 8-byte values.
//!
//! This is the generalisation of [`crate::dense_writeback::DenseWriteBehindU64`]:
//! the key type is generic (`K: Eq + Hash + Clone`) and the value type can be any
//! [`crate::DenseValue`] type (`u64`, `i64`, `f64`, `[u8; 8]`, etc.).
//!
//! The table keeps `ConcurrentMap<K, packed_slot_handle>` as a sparse index, and
//! stores the hot value/version/dirty state in pre-allocated slot arrays. Existing
//! key updates do not allocate or retire hash-table entries; they update the slot
//! directly and optionally enqueue one dirty record.

use crate::DenseValue;
use crate::bucketed_index::{BucketedIndex, BucketedInsertOutcome};
...
pub struct DenseWriteBehindMap<K, V, S = FastBuildHasher>
where
    K: Eq + Hash + Clone + Send + Sync + 'static,
    V: DenseValue,
    S: BuildHasher,
{
    index: BucketedIndex<K, S>,
    ...
    S: BuildHasher + Clone,
{
    pub fn with_hasher_and_config(hash_builder: S, config: WriteBehindConfig) -> Self {
        let cap = u32::try_from(config.initial_capacity.max(1))
            .expect("DenseWriteBehindMap capacity exceeds u32::MAX");
        ...
        let parks: Vec<_> = (0..dirty_shards)
            ...
            .collect();
        Self {
            index: BucketedIndex::with_hasher_and_capacity(hash_builder.clone(), cap as usize),
    ...
    fn cleanup_after_flush(&self, drained: &[DrainedEntry<K>]) {
        for (_, entry) in drained {
            ...
            let Some(handle) = self.index.get(&entry.key) else {
                continue;
            };
        ...
        F: FnMut(usize) -> B,
    {
        ...
        for shard_idx in 0..shards {
            ...
            handles.push(std::thread::spawn(move || {
                run_dense_map_flusher_loop(shard_idx, cache, backend, stop, cfg);
            }));
        }
        Self {
            handles,
...
fn run_dense_map_flusher_loop<K, V, S, B>(
    shard_idx: usize,
    cache: Arc<DenseWriteBehindMap<K, V, S>>,
    mut backend: B,
    stop: Arc<std::sync::atomic::AtomicBool>,
...
where
    ...
    S: BuildHasher,
{
    cache: &'g DenseWriteBehindMap<K, V, S>,
    index: &'g BucketedIndex<K, S>,
}
    ...
    S: BuildHasher + Clone,
{
    ...
    pub fn put(&self, key: K, value: V) -> u64 {
        match self.write(key, value, true) {
            Ok(version) => version,
            Err(_) => panic!("DenseWriteBehindMap is full"),
        }
    }
    ...
    fn write(&self, key: K, value: V, queue_dirty: bool) -> Result<u64, V> {
        ...
        match self.index.insert_returning_old(key.clone(), handle) {
            (BucketedInsertOutcome::Inserted | BucketedInsertOutcome::Replaced, old_handle) => {
                self.cache.unlock_slot(slot, generation);
            ...
            (BucketedInsertOutcome::Full, old_handle) => {
                self.cache.unlock_slot(slot, generation);
                if let Some(old_handle) = old_handle {
                    self.cache.retire_slot(old_handle);
                }
...
mod tests {
    ...
    fn generic_key_and_value_round_trip() {
        let cache = DenseWriteBehindMap::<String, [u8; 8]>::with_capacity(16);
    ...
    fn delete_emits_delete_record() {
        let cache = DenseWriteBehindMap::<String, u64>::with_capacity(16);
    ...
    fn auto_flusher_drains_generic_writes_in_background() {
        ...
        let cache = Arc::new(DenseWriteBehindMap::<String, [u8; 8]>::with_config(
            WriteBehindConfig {
                dirty_shards: 2,
                initial_capacity: 64,
    ...
    fn concurrent_readers_do_not_observe_none_during_puts() {
        ...
        let cache = Arc::new(DenseWriteBehindMap::<u64, [u8; 8]>::with_capacity(16));
